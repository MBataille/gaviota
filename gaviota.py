...
# Pajaro creado
gaviota = objeto(mesh.Mesh.from_file('bird.stl'),color.white)
gaviota.set_pos(vector(r,0,2*r))
gaviota.rotar_x(np.pi,gaviota.cdg)
gaviota.rotar_y(np.pi-beta,gaviota.cdg)
gaviota2 = sphere(pos=gaviota.cdg, radius=r/30, color=color.red,
...
rarr = arrow(pos=gaviota2.pos, axis=vector(1,0,0)*r/5, color=color.magenta)
...
tarr = arrow(pos=gaviota2.pos, axis=vector(0,1,0)*r/5, color=color.white)
...
while True:
	for this_t in t:
		...
		Larr.axis = -np.sin(beta)*rarr.axis*1.2 + np.cos(beta)*z_k.axis*1.2	
